...
class VIudp
  ...
  def acceptConnection(receiver_port)
    ...
    @sender = UDPSocket.new
    @sender.bind("localhost", @sender_port)
    #first handshake receiving connection message
    ...
    if (connection_msg.include? "Connecting")
      ...
      @sender.send(public_RSA_key.to_s, 0, "localhost", @receiver_port)
      ...
      @sender.send("The key was received...", 0, "localhost", @receiver_port)
      return @sender
